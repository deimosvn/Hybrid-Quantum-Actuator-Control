...
"""Sistema de Control Híbrido Cuántico-Clásico para Rover (demo standalone).

Punto de entrada que ejecuta el sistema SIN ROS 2, útil para validar los
algoritmos de control de forma aislada. Integra:
...
Ejecutar con:  ros2 run quantum_rover_control sim_demo
(o directamente:  python -m quantum_rover_control.sim_demo)
"""
...
# Importaciones locales (paquete ROS-agnóstico)
    ...
    def control_step(self) -> Tuple[Dict, TelemetryRecord]:
        ...
        try:
            ...
            cost = self.optimizer.cost_function(
                np.array([float(error_state[0]), float(error_state[1]),
                          float(control_normalized)]),
                self.optimizer.Q,
                self.optimizer.R
            )
...
def main():
    """Ejecuta la demostración standalone completa del sistema."""
