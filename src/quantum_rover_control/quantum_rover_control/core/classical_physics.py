...
Ecuaciones:
    Motor: tau = K_t * I = K_t * (V - K_e * w) / R
    Dinámica: J * dw/dt = tau - tau_friction - tau_slip
    Error: e = w_ref - w_actual
"""
...
class MotorSimulator:
    ...
    def _motor_dynamics(self, state: np.ndarray, V: float,
                        torque_external: float = 0.0) -> np.ndarray:
        """
        ...
        # Corriente del motor: i = (V - K_e * w) / R
        current = (V_clipped - self.params.K_e * omega) / self.params.R

        # Torque motor: tau_motor = K_t * i
        tau_motor = self.params.K_t * current
    ...
    def step(self, V: float, torque_external: float = 0.0) -> Tuple[float, float]:
        """Integración numérica del motor (1 paso de simulación, Euler explícito).
...
class RoverDynamics:
    ...
    def _calculate_traction_loss(self, omega: float) -> float:
        ...
        Returns:
            Torque perdido (N·m)
        """
        # Torque de deslizamiento: tau_slip = m * g * mu * r * traction_loss_factor
