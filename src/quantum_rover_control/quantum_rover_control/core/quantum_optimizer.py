...
# Qiskit is an optional dependency. The variational parameters themselves are
# optimized with SciPy's COBYLA, so we only need the circuit builder plus the
# Aer simulator from Qiskit. This block stays compatible with both Qiskit 0.4x
# and Qiskit >= 1.0 (where ``qiskit.algorithms`` was removed).
try:
...
class QAOAController(QuantumOptimizer):
    ...
    def cost_function(self, x: np.ndarray, Q: np.ndarray, R: np.ndarray) -> float:
        """Calcula el costo del control: J = ||e||_Q^2 + ||u||_R^2.
    ...
    def _optimize_quantum(self, error_state: np.ndarray) -> float:
        ...
        # Optimización clásica de los parámetros variacionales (COBYLA de SciPy)
    ...
    def update_cost_matrices(self, Q: Optional[np.ndarray] = None,
        ...
        if R is not None:
            self.R = R
        logger.info(
            f"Matrices de costo actualizadas: Q shape={
                self.Q.shape}, R shape={
                self.R.shape}")
